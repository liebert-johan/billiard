...
def create_ball(radius, pos):
    ...
    # добавляем трение с помощью метода из pymunk
    pivot = pymunk.PivotJoint(static_body, body, (0, 0), (0, 0))
    pivot.max_bias = 0
    pivot.max_force = 1000  # сила удара
...
# создаем шкалу силы нашего удара
...
while run:
    ...
    # нарисуем бильярдный стол
    screen.blit(table_image, (0, 0))

    # проверяем на наличие забитых шаров
    # делал Роман Малиев
    for i, ball in enumerate(balls):
        for pocket in pockets:
            ...
            if ball_dist <= pocket_dia / 2:
                # проверяем был ли забитый шар кийем
                if i == len(balls) - 1:
                    lives -= 1
    ...
    # проверям остнаовились ли все шары
    taking_shot = True
    for ball in balls:
    ...
    if taking_shot == True and game_running == True:
        if cue_ball_potted == True:
            ...
            cue_ball_potted = False
        # вычисляем угол отскока кия
        mouse_pos = pygame.mouse.get_pos()
        cue.rect.center = balls[-1].body.position
        x_dist = balls[-1].body.position[0] - mouse_pos[0]
        y_dist = -(balls[-1].body.position[1] - mouse_pos[
            1])  # так потому что в pymunk координаты перемещаются
    ...
    # рисуем уже забитые мячи на этой панели
    ...
    # проверям были ли все мячи забиты
    if len(balls) == 1:
        draw_text("YOU WIN!", large_font, WHITE, SCREEN_WIDTH / 2 - 160, SCREEN_HEIGHT / 2 - 100)
        game_running = False

    # все работает если выполняются эти условия
